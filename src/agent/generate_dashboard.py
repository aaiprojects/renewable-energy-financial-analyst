...
def get_latest_archive_dir(archive_root: Path = Path("archive")) -> Path | None:
    """Return the most recent archive directory (supports both date and timestamp formats)."""
    if not archive_root.exists():
        print("⚠️ No archive root found.")
        return None

    # Capture both YYYY-MM-DD and YYYYMMDD_HHMM formats
    dirs = [d for d in archive_root.iterdir() if d.is_dir()]
    if not dirs:
        print("⚠️ No archive subdirectories found.")
        return None

    # Sort by last modified time instead of name, to handle mixed formats
    dirs.sort(key=lambda p: p.stat().st_mtime, reverse=True)

    latest = dirs[0]
    print(f"📦 Using latest archive folder: {latest.name}")
    return latest
...
def load_current_and_previous_runs() -> tuple[dict, dict, Path | None]:
    ...
    Current = executive summaries from current working directory
    Previous = summaries from latest archive folder (auto-detected)
    """
    ...
    print(f"✅ Loaded {len(current)} current summaries, {len(previous)} archived summaries from {prev_dir}")
...
def compute_confidence_deltas(current: dict, previous: dict) -> pd.DataFrame:
    ...
    if df.empty:
        print("⚠️ No data found for confidence deltas — returning empty DataFrame.")
